// ----------------------------------------------------------------------------
// <copyright file="PhotonAnimatorViewCustomEditor.cs" company="Exit Games GmbH">
...
namespace Photon.Pun
{
    ...
    [CustomEditor(typeof(PhotonAnimatorViewCustom))]
    public class PhotonAnimatorViewCustomEditor : Editor
    {
        private Animator m_Animator;
        private PhotonAnimatorViewCustom m_Target;
        ...
        {
            this.m_Target = (PhotonAnimatorViewCustom)this.target;
        ...
        private void DrawWeightInspector()
        {
            ...
            for (int i = 0; i < this.GetLayerCount(); ++i)
            {
                ...
                {
                    this.m_Target.SetLayerSynchronized(i, PhotonAnimatorViewCustom.SynchronizeType.Disabled);
                }

                PhotonAnimatorViewCustom.SynchronizeType syncType = this.m_Target.GetLayerSynchronizeType(i);
                ...
                syncType = (PhotonAnimatorViewCustom.SynchronizeType)EditorGUI.EnumPopup(popupRect, syncType);
        ...
        private void DrawParameterInspector()
        {
            ...
            for (int i = 0; i < this.GetParameterCount(); i++)
            {
                ...
                {
                    this.m_Target.SetParameterSynchronized(parameter.name, (PhotonAnimatorViewCustom.ParameterType)parameter.type, PhotonAnimatorViewCustom.SynchronizeType.Disabled);
                }

                PhotonAnimatorViewCustom.SynchronizeType value = this.m_Target.GetParameterSynchronizeType(parameter.name);

                // check if using trigger and actually synchronizing it
                if (value != PhotonAnimatorViewCustom.SynchronizeType.Disabled && parameter.type == AnimatorControllerParameterType.Trigger)
                {
                ...
                Rect popupRect = new Rect(elementRect.xMin + EditorGUIUtility.labelWidth, elementRect.yMin + 2, elementRect.width - EditorGUIUtility.labelWidth - 5, EditorGUIUtility.singleLineHeight);
                value = (PhotonAnimatorViewCustom.SynchronizeType)EditorGUI.EnumPopup(popupRect, value);
                ...
                if (value != this.m_Target.GetParameterSynchronizeType(parameter.name))
                {
                    ...
                    this.m_Target.SetParameterSynchronized(parameter.name, (PhotonAnimatorViewCustom.ParameterType)parameter.type, value);
                }
            }
